...
## 2026-09-18 (DEM rung 0) — the cheapest check on a claim is usually the one where the truth is already known

- **I reached for a 200-row hand-check before running the 32-row calibration that settled the question for free.** The normalizer's whole risk is precision, and the synonym table's own keys are labelled data I had written myself an hour earlier. Running the rule over them took one call and produced the decisive result (26/32, with `first name` -> `birthday` at 0.834); the hand-check then only confirmed it. **Next time a mapping/classifier has to be evaluated, list the inputs whose answer is already known BEFORE designing a sample.** `active`.
- **Two "unexplained variance" signals in one session both turned out to be the instrument, and both were settled in minutes by asking the instrument directly.** 24/100 tables differing where 2 could; two identical ablation runs scoring differently. In both cases the fastest path was not a hypothesis but a direct determinism probe (call the reranker twice; ingest the same question twice). **When a number moves more than the treatment can explain, probe the tool before theorising** — it is cheaper than every alternative and it either exonerates the tool or hands you a bigger finding than the one you were chasing. Here it handed over the biggest result of the rung. `active`.
- **I wrote a flush-at-the-end loop the same morning I read the memory entry warning about exactly that.** Reading a lesson and applying it are different acts; the lesson was filed under "cache format" and my bug was in "cache timing", so the index I had built from it did not fire. When a memory entry describes a failure, extract the PROPERTY it protects (nothing expensive is unrecoverable) rather than the shape it took.
