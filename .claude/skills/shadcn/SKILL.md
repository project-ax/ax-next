...
>
> **Monorepo workspace flag (ax-next):** This is a pnpm monorepo. The shadcn workspace is `packages/channel-web`. **Append `-c packages/channel-web` to every shadcn CLI invocation** in this repo. Without it, the CLI errors with `{"error":"monorepo_root", ...}`. The examples throughout this file omit the flag for readability — add it whenever you actually run a command. e.g. `npx shadcn@latest add button` becomes `npx shadcn@latest add button -c packages/channel-web`.

## Current Project Context

```json
!`npx shadcn@latest info --json -c packages/channel-web`
```

The JSON above contains the project config and installed components. Use `npx shadcn@latest docs <component> -c packages/channel-web` to get documentation and example URLs for any component.
