...
- **No clock by default.** A caller that cannot afford to wait forever passes `hooks.fire(event, ctx, payload, { subscriberTimeoutMs })`: each subscriber still running at the bound is logged as `hook_subscriber_timed_out` and skipped (its late result discarded, a late throw still reported with `timedOut: true`). It is not cancelled — it keeps running — so the bound limits its say over this fire, not its side effects. `chat:start` is bounded this way (`CHAT_START_SUBSCRIBER_TIMEOUT_MS`, TASK-514), and so is every `chat:end` / `chat:turn-error` / `chat:permission-request` the orchestrator fires (`CHAT_EVENT_SUBSCRIBER_TIMEOUT_MS`, TASK-551). The runner-reported `chat:end` fired by `@ax/ipc-core` is deliberately left unbounded: bounding it changes `fire()`'s microtask timing on every successful turn's path, which is what broke preset-k8s acceptance in TASK-514.
