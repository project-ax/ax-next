...
- **No clock by default.** A caller that cannot afford to wait forever passes `hooks.fire(event, ctx, payload, { subscriberTimeoutMs })`: each subscriber still running at the bound is logged as `hook_subscriber_timed_out` and skipped (its late result discarded, a late throw still reported with `timedOut: true`). **Every subscriber receives a third argument, `{ signal }`, and the bus aborts that signal at the bound** (TASK-552; reason is a `TimeoutError` `DOMException`). JavaScript cannot cancel a promise, so a subscriber that ignores its signal keeps running — the bound guarantees only its say over this fire. A subscriber doing long or side-effecting work (storage writes, network, model calls) should check `signal.aborted` before each committing step; stopping via `signal.throwIfAborted()` is logged at debug (`hook_subscriber_aborted`), not as a failure. memory-strata's chat:start bootstrap is the reference honouring subscriber. The signal is deliberately not on `AgentContext` (services don't honour one). `chat:start` is bounded this way (`CHAT_START_SUBSCRIBER_TIMEOUT_MS`, TASK-514), and so is every `chat:end` / `chat:turn-error` / `chat:permission-request` the orchestrator fires (`CHAT_EVENT_SUBSCRIBER_TIMEOUT_MS`, TASK-551). The runner-reported `chat:end` fired by `@ax/ipc-core` is bounded too (`RUNNER_CHAT_END_SUBSCRIBER_TIMEOUT_MS`, TASK-555). **Trap when bounding a fire that races its caller:** under a bound, a subscriber registered after the one that resolves the caller may run after the caller has resumed (TASK-514 hit this in preset-k8s acceptance). Assert "exactly once" after the turn settles (wait for it, then shut down, then count), never on return.
