...
- [ ] **Retire `@ax/auth-oidc` entirely** in favor of `@ax/auth-better`. Today auth-oidc still owns four hook registrations (`auth:require-user`, `auth:get-user`, `auth:create-bootstrap-user`, `auth:complete-bootstrap-user`), exports the `HttpRequestLike` + session types that `@ax/auth-better` type-imports (`packages/auth-better/src/plugin.ts:31`, index re-export at `auth-better/src/index.ts:13`), and the `createAuthPlugin` factory is wired into ~8 test files across `agents` / `teams` / `mcp-client` / `onboarding` / `auth-better`. Retirement needs: (a) port the four hook registrations into auth-better (or remove them — auth-better has its own session model), (b) rehome the shared types into auth-better or a dedicated types package, (c) migrate test fixtures off `createAuthPlugin`, (d) drop the `@ax/auth-oidc` workspace dep from every consumer. Unblocks the dev-bootstrap deletion below. Pick up when auth-better is feature-complete enough to be the sole auth surface.
- [ ] **Delete `/auth/dev-bootstrap` route + `dev-bootstrap.ts`** in `@ax/auth-oidc`. Falls out automatically when the parent auth-oidc retirement above ships; listed separately so a future incremental approach (delete dev-bootstrap first, retire the rest later) is still a coherent option.

## Credentials UX redesign (PR #109, merged 2026-05-19)

- [x] ~~**ModelConfigTab broken at runtime.**~~ Shipped in this branch — new `@ax/admin-settings-routes` plugin owns `GET/PUT /admin/settings/:key` (allowlist: `fast-model`); `@ax/conversation-titles` reads `settings:fast-model` at every `chat:turn-end` and falls back to `cfg.model` when storage is absent; the wizard's `completion-tx` writes the canonical `anthropic/<model>` ref; ModelConfigTab drops the unused `runner-model` row and uses the new endpoint. Wired into the k8s preset.
