...
**Architecture (as shipped — note the Codex-review evolution below):** The transport's `processResponseStream` `flush()` originally synthesized a `finish`/`stop` chunk on a `done`-less close (the bug). The first draft changed it to emit an AI-SDK `error` chunk (`CONNECTION_LOST`) and had the runtime silently `regenerate()` once. **Codex review (round 4, P1) showed that a client-side `regenerate()` re-POSTs and can DUPLICATE a still-running server turn** (a client SSE disconnect does not terminate the runner; every POST mints a fresh reqId + `agent:invoke`). An intermediate draft moved silent recovery into the transport as a same-reqId GET reconnect, but **Codex rounds 4–8 established that no client-side reconnect can be loss-free**: a re-POST duplicates a live server turn (P1), and a GET-only buffer-replay reconnect can silently drop output whenever the agent outruns the server's bounded 256-chunk ring buffer while the client is disconnected (the wire has no per-chunk sequence number to align a partial replay) — true even for a pre-content reconnect. Since silent data loss is unacceptable for this project, the **shipped design does NOT auto-reconnect or auto-regenerate**: on any mid-turn drop (graceful done-less close OR hard body error), `buildTurnStream` emits the `CONNECTION_LOST` `error` chunk → the existing `AgentStatus` error banner with a manual-retry (`regenerate`) affordance. A server `error` frame (Fault A) keeps its mapped label; an abort (Stop/teardown) closes cleanly with no banner; a clean `done` finishes normally. One POST + one GET per turn, always. The runtime's `onError` is just `applyTurnError` (banner). No new UI components — reuses the existing `AgentStatus` row. This fully fixes the FAULTA-5 bug (no more silent finalize) and delivers the required "error banner with a retry affordance". **Follow-up (handoff → auto-ship):** a server-side per-chunk sequence number on the SSE wire (SseFrame + chunk-buffer + the `chat:stream-chunk` producer) would let the client dedup an arbitrary partial replay exactly and resume silently mid-turn without loss — the correct way to add the transparent "silent retry first" the clarification asked for. See `.claude/memory/decisions.md` (2026-05-25 FAULTA-5 entries) for the full round-by-round rationale.
