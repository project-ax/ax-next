...
**Architecture:** The transport's `processResponseStream` `flush()` currently synthesizes a `finish`/`stop` chunk on a `done`-less close (the bug). Change it to emit an AI-SDK `error` chunk carrying a dedicated shared sentinel (`CONNECTION_LOST`). The runtime's `onError` distinguishes that sentinel from orchestrator-terminated errors: on the FIRST connection-lost error per turn it silently `regenerate()`s (transient working-mode "Connection lost — retrying…" label, no banner); on a second failure (or any other error) it falls through to the existing `applyTurnError` error banner. The retry-decision logic is extracted into a pure, unit-testable helper (mirroring `applyTurnError`), because the runtime test harness mocks `useChat`. No new UI components — reuses the existing `AgentStatus` row (shadcn semantic tokens already in place).
