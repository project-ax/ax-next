...
## 12. DECIDED PLAN — this is the order to execute (ruled 2026-08-24)

§11 lays out the options; **this is the choice.** The lead took **(a)**, sequenced ahead of further
draining. Execute in this order.

### Step 1 — Remediate the rot that exists, and detect it with the thing that prevents it

**Key insight: the guard IS the inventory.** A `scripts/__tests__/` guard that asserts every symbol
and path named in `.claude/memory/*.md` still exists will, on its **first red run**, enumerate the
existing rot. So remediation and prevention are **one sequence, not two work streams** — build the
detector, let it produce the list, fix what it finds.

That ordering matters: an unguided "audit the memory files" sweep would be unbounded and would itself
produce claims nobody verifies. The guard bounds it to claims that are *mechanically checkable* and
leaves a permanent tripwire behind.

### Step 2 — Close loop 3 so new rot stops being generated

Two conventions, both cheap, both from this session's evidence:
- **`yolo-ship` definition of done:** *if a stale doc/memory line produced this card, fixing that line
  is part of the card.* Currently this is something the orchestrator asks for per-card, which means it
  is missed whenever nobody thinks to ask. Three cards this session were generated by stale lines.
- **`.claude/memory/` convention: cite the file, not the line, unless the line IS the point.** Every
  wrong `file:line` this session sat in prose that did not need one. TASK-310's builder adopted this
  unprompted and it is the cheapest structural fix available.

### Step 3 — Drain, in this order
...
the wrong instrument, a global `fileParallelism:false`, adds ~15 min to a 7.5–9.5 min job). And
**313**, which is spec but wants the `ux-design` skill for final copy.

### Step 4 — Re-measure, then decide the exit condition

After one more session, compute **cards filed : cards merged**. Below 1:1 ⇒ the epic is genuinely
converging, keep going. Not below ⇒ take §11(c) and **time-box it**: close the epic and let the
remainder compete with everything else on the board. Do not let this run indefinitely on the
assumption that the next wave is the last one — that assumption has now been wrong four sessions
running.

### Still open — needs a human answer, blocks one card

**Are pinned OAuth endpoints (`authServerUrl` / `tokenUrl`) a supported, reach-relevant configuration,
or effectively always DCR-derived?** TASK-262's reviewer flagged that its Minor finding becomes
**Important** if the former. Until answered, the deferred digest gap (§9) is **not carded**, on
purpose — filing it at the wrong severity is how a card ends up wrong about its own bug.
