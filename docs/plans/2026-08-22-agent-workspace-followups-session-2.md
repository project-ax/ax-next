...
| TASK-269 | Dead `ToolGroup` / `headerPhrase` / `toolVerb` / `VERB_MAP` in `ToolUse.tsx` — unreferenced outside their own test. TASK-260's card told its builder to edit them; both edits would have been **no-ops nobody renders**. **Resolved 2026-08-24: deleted, not revived** — `ChainOfThought.tsx` already was that disclosure. |
