...
These are different shapes. Neither is derived from the other: the jsonl is lossy for display (no host UI events), the display log is lossy for resume (no SDK bookkeeping). This is *better* on I4, not worse — **display** and **resume** are each authoritative for a different audience: the jsonl for **what the model saw** (raw, byte-exact), the event log for **what the user saw** (redacted, host-annotated). The model/tool *content* appears in both, as two *views* of the same turn — and that's fine, because the two views are **supposed** to differ where the audiences differ (a masked credential, a host-only error). The risk this creates, and how it's bounded, is settled in **B3 below**: the enemy is *omission*, not *divergence*.
...
### B3. Divergence: two audiences, one enemy (omission)

Two stores serializing the model/tool turns twice looks like a dual-write hazard. It mostly isn't — and the part that is, is bounded by a smoke detector, not a consistency protocol. This subsection replaces any "keep them byte-identical / divergence is a bug" reading.

**First, the dual *store* is required, not gold-plating.** Resume needs the byte-exact jsonl *with* SDK bookkeeping (`queue-operation`, `last-prompt`) and *without* host UI; display needs the host UI *and* the model/tool turns and does *not* want SDK bookkeeping. Neither can serve the other's job. Two stores is the floor, not a choice.

**The frames-as-display-SoT choice (B1) is deliberate, and we keep it.** The alternative — store only the jsonl + a sparse host-event sidecar and *merge at read* — would give one serialization of model/tool content but **two render paths** (live = fold the SSE stream; reload = merge-then-fold), trading data-divergence for render-divergence. Persisting and replaying the exact frames keeps **one render path** ("reload == live by construction"), which is the stronger property. We pay for it with two serializations of the model/tool turns — bounded below.

**The reframe: the two logs are authoritative for different audiences, so most divergence is *correct*.** A credential masked in display but real in resume; a sandbox-death error in display but never in resume; SDK bookkeeping in resume but never in display — all correct, none a bug. Byte-identity was never the requirement.

**The actual enemy is *omission*** — a *completed* turn missing from display (the user can't see their own chat) or from resume (model amnesia). That's prevented by a far weaker, achievable invariant than dual-write consistency:

1. **No omission — persist-before-ack.** Each store durably persists its complete record for a turn before that turn is acknowledged. B2 already does this (bind-after-`append-transcript`-durable, mirroring today's bind-after-commit-`accepted`); B1 must persist the turn's frames before the turn-end ack, the same discipline. A completed turn is never absent from either store.
2. **Content-divergence is expected and correct-by-audience.** Display = what the user saw; resume = what the model saw. They need not match byte-for-byte and are not "fixed" from each other.

**The detector (cheap, structural — not a hash, not derivation).** At turn-end, cross-check **turn count + role/order sequence** (N user/assistant/tool turns, same order) between the display log and the resume rows; a mismatch is a **loud assertion/metric**, never silent. It's structural on purpose: a content hash would false-positive on legitimate redaction divergence, whereas counting turn boundaries catches the only thing that's actually a bug (a dropped/omitted turn) without fighting correct-by-audience differences.

**Deferred escalation (YAGNI).** If that detector fires in practice, escalate to **deriving** the display model/tool turns from the resume jsonl (the runner re-projects at turn-end) for divergence-free-by-construction — accepting the extra cost then. Until it fires, don't build it: the structural detector is the smoke alarm; derivation is the heavy hammer. Install the alarm now; swing the hammer only if it rings.
...
- **I4 (one source of truth).** The big win, three places. Transcript splits into two SoTs authoritative for **different audiences** (display log = what the user saw; resume rows = what the model saw), each with one writer (the runner); they overlap in content as two *views* of a turn, governed by B3's no-omission invariant + structural detector rather than a byte-identity requirement. Blobs: content-addressed, one store. Skills: one DB+blob store for catalog/default/attached/authored — kills the draft-in-git / rows-in-DB / git-tree-bundle split-brain. Removes the transcript/attachments shared-mirror contention that caused the loss bug.
...
2. **Phase 2 — transcript out of git.** Two halves. **2a (display):** persist the `event.turn-end`/`event.stream-chunk` frames the host already receives into a `conversation_events` log; switch `conversations:get` to read it (no jsonl parse). **2b (resume):** add `session.append-transcript`/`replace-transcript` + the row-per-line `conversation_transcripts` store; replace `commitTurnAndBundle` at the `result` boundary with the delta-ship (keeping `waitForTranscriptUuid`); switch resume to rebuild the jsonl from rows; F2a guard becomes `max(seq) > 0`. Land the **B3 invariants** with this phase: persist-before-ack on both stores, and the structural turn-count/order divergence detector (loud, not silent). **Biggest robustness win** — re-walk the latency + concurrent-writer + resume scenarios on `ax-next-dev`, and confirm the detector stays quiet across them.
