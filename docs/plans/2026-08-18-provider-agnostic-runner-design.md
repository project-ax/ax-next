...
| 7 | ✅ **Merged 2026-08-21 (#423, `b04a974a`).** Compaction rung 3 (summarize) + eval suite | Gate met, and walked: on kind, a 124-message conversation compacted to 41 (`~188,266 of 200,000 tokens before, ~63,768 after`) and a fact stated ONCE inside the summarized middle came back correct in the UI — including after a cold resume that re-read the compacted transcript. Post-compaction answerability holds. Rung 3 fires at the TURN BOUNDARY (see §7) and is the one rung that rewrites the transcript — it costs a model call, which cannot be recomputed for free the way a mask or a prune can. The rewrite is published through a new `LoopContext.replaceTranscript` over the existing `session.replace-transcript` action, per §5, rather than being left for the delta ship's `resync-required` path to discover. The eval is `pnpm --filter @ax/agent-aisdk-runner bench` — a needle corpus with a **verbatim control arm**, so the gate is the recall the summary COST rather than an absolute the filler's difficulty would dominate. Its harness is unit-tested in CI; the model half is opt-in on a real key. |

**Sequence complete** as of 2026-08-21, except row 5 (Vertex), skipped deliberately. Rows 1–4 and
6–7 are merged and the §8 walks have been run against each. Picking Vertex up later is still one
`PROVIDERS` entry plus a credential kind — nothing in the compaction work assumes a provider set.
