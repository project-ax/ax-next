...
`skills:approved-caps-list`.* Note that `skills:approved-caps-list` currently has **no
production caller** (its declared degradation at `packages/agents/src/plugin.ts:142-145`
describes behaviour that no longer runs); AW-14 would be its first real reader.
