...
4. **Vector retrieval is now conditional on benchmark evidence.** ByteRover's strong LoCoMo numbers using BM25-only retrieval prompted a deliberate eval spike (see "Evaluation Plan"). If BM25 + LLM rerank performs comparably to BM25 + dense + RRF, we drop the vector path and the embedding-model dependency entirely.
