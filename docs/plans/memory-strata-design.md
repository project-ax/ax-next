...
2. **Indexer layer adopts memsearch.** Rather than building hybrid retrieval from scratch, the implementation either ports memsearch's indexer to TS or runs it as a Python sidecar. The Retriever section below treats memsearch as the implementation; the choice of port-vs-sidecar is deferred to the Phase 1 handoff brief.
...
4. **Vector retrieval was conditional on benchmark evidence and is now dropped.** ByteRover's strong LoCoMo numbers using BM25-only retrieval prompted a deliberate eval spike (see "Evaluation Plan"). The Phase 3B binding decision (2026-05-13) confirmed: BM25-only beats BM25+dense+RRF by 9 points on LongMemEval-S. Strata production indexers stay BM25-only — no embedding-model dependency, no vector index, no RRF fusion layer.
...
- Three thermal tiers with explicit token budgets (hot ~1500 default / 2500 cap; warm ~100-doc LRU)
...
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │  🔴 HOT TIER  (always in context, ~1500 default / 2500 cap) │  │
│  │                                                            │  │
│  │  ┌──────────┐  ┌──────────┐  ┌────────────────────────┐  │  │
...
│   ├── session.md            # Rolling summary of current session
...
Full body text is indexed in FTS5 for keyword search but is *not* the primary embedding target. Embedding the summary field and headers produces a dramatically smaller vector index while retaining high retrieval relevance. Full section text is loaded on-demand only when the agent explicitly requests it.
...
### 2. Retriever (from Hybrid RAG, implementation via memsearch)

**Implementation note (2026-05-10):** the hybrid retrieval mechanics described below are implemented by [memsearch](https://github.com/zilliztech/memsearch) (MIT) — markdown-source-of-truth + content-hash incremental indexing + dense/BM25/RRF retriever. The pseudocode here describes Strata's *interface* over the indexer; the indexer itself is memsearch (port-to-TS or Python sidecar — decision deferred to the Phase 1 handoff brief). What Strata adds on top: tier-aware result routing (system → warm-cache → cold-search), summary-first / header-mode shaping, and frontmatter-scoped indexing rather than full-body.

Every retrieval query runs through **both** sparse and dense search, then fuses results. The retriever uses a combined scoring formula:

```
final_score = 0.35 × lexical + 0.35 × dense + 0.15 × recency + 0.10 × importance + 0.05 × scope_match
```

This multi-signal approach vastly outperforms vector-only search, especially for markdown corpora where exact names, IDs, acronyms, and headings matter as much as semantic meaning.

```python
class Retriever:
    """
    Hybrid search: BM25 + vector similarity + metadata scoring.
    """

    def search(self, query: str, filters: dict = None,
               top_k: int = 5, mode: str = "summary") -> list[Result]:
        # 1. Sparse search (keyword/BM25 via SQLite FTS5)
        bm25_results = self.fts_index.search(query, limit=top_k * 2)

        # 2. Dense search (vector similarity over summaries + headers)
        query_embedding = self.embed(query)
        vector_results = self.vector_index.search(
            query_embedding, limit=top_k * 2, filters=filters
        )

        # 3. Reciprocal Rank Fusion
        fused = self.rrf_merge(bm25_results, vector_results)

        # 4. Apply recency/importance/scope scoring
        scored = self.apply_meta_scores(fused)

        # 5. Optional: LLM rerank for top candidates
        if len(scored) > top_k:
            scored = self.rerank(query, scored[:top_k * 2])

        # 6. Return based on mode
        results = scored[:top_k]
        if mode == "summary":
            # Return only frontmatter summaries (~50 tokens each)
            return [Result(doc.id, doc.title, doc.summary) for doc in results]
        elif mode == "headers":
            # Return document header tree for drill-down
            return [Result(doc.id, doc.title, doc.headers) for doc in results]
        else:
            # Return full content
            return results

        # 7. Update access metadata (feeds Promoter)
        for doc in results:
...
Total: ~1500-2500 tokens base, even with
...
This means **at any given time**, the context contains only: hot memory (~500 tokens) + auto-injected summaries (~200 tokens) + one actively loaded section (~200 tokens) + conversation + tools ≈ **well under 4K tokens**, regardless of archive size.
...
| Cold tier | SQLite FTS5 for keyword search (stdlib). A small vector index (e.g., `sqlite-vec`, `hnswlib`, or NumPy brute-force for <10K docs) |
...
| Retriever | Two queries (FTS5 + vector) + RRF merge (~30 lines) |
...
1. **Hot tier budget**: Default ~1500 tokens, hard cap 2500 tokens (`agent.md` + `user.md` + `session.md` + `recent.md` + scratchpad combined). `session.md` is a compressed rolling summary, not full history. Scratchpad is cleared between tasks. When the cap is exceeded, the Consolidator compresses in priority order: oldest paragraphs of `session.md` → low-importance items in `user.md` → low-`access_count` entries in `recent.md`. `agent.md` is never compressed (persona is fixed-cost).
...
- Abstention accuracy (agent says "I don't know" rather than confabulating)
...
ByteRover reports strong LoCoMo results without vector retrieval (BM25 + LLM rerank only). Before committing Strata to dense embeddings (an embedding-model dependency, an extra index, write-time embedding latency, and re-embed cost on schema changes), run a head-to-head:
...
| **C: BM25 + dense + RRF** (current spec) | embedding model + vector index | full memsearch hybrid |

If A or B comes within ~3 points of C on LongMemEval-S, drop C — the embedding dependency is not earning its keep. This decision is gated behind eval data, not opinion.

**Result (2026-05-13 Phase 3B spike):** Level 3 OUT. On 100-Q LongMemEval-S (Sonnet 4.6 + Grok 4.3 judge): A = 22.0%, B = 19.6%, C = 13.0%. C loses to A by 9 points and to B by 6.6 points — clearly outside the ±3-point band. B also underperforms A by 2.4 points, so the Phase 4 reranker (Level 6) priority is reconsidered alongside vectors. See `docs/plans/2026-05-13-memory-strata-vector-spike-report.md`.
...
LEVEL 0: Just hot tier files + conversation
         (Works with zero infrastructure—just read 3 .md files)
              │
              ▼
LEVEL 1: Add Observer for auto-extraction to inbox/ and docs/
...
LEVEL 2: Add SQLite FTS5 index for keyword search
         (One pip install, zero config)
              │
              ▼
LEVEL 3: (OUT — Phase 3B spike showed BM25-only beats BM25+dense+RRF
         by 9 points on LongMemEval-S accuracy; the lightweight
         summary+headers vector index doesn't earn its keep. See
         `2026-05-13-memory-strata-vector-spike-report.md`.)
         The Strata production indexers stay at Level 2 (BM25 over body)
         and skip the embedding-model dependency entirely.
              │
              ▼
LEVEL 4: Add Consolidator for periodic maintenance
         (A cron job or session-end hook)
              │
              ▼
LEVEL 5: Add warm tier LRU cache + Promoter for access-pattern optimization
...
You can stop at any level and have a functional system. Each level adds measurable improvement. Level 0–2 can be built in an afternoon. Level 3–4 is a solid production system. Level 5–6 is optimization.
...
| Zone 2: User profile | ~300 | Session-stable | 19/20 | 1 (first turn) |
...
$$\text{Total input tokens} = 2500 \times 20 = 50{,}000 \text{ tokens}$$
...
$$\text{Turn cost} = \underbrace{800 \times 0.1}_{\text{Zone 1 cached}} + \underbrace{300 \times 0.1}_{\text{Zone 2 cached}} + \underbrace{400 \times p_3}_{\text{Zone 3}} + \underbrace{1000 \times 1.0}_{\text{Zone 4 full}}$$

Where $p_3 \approx 0.1$ for 80% of turns (cached) and $1.0$ for 20% (after session.md updates).

**Average turn:** $80 + 30 + 40 + 1000 = 1{,}150$ effective tokens
**Over 20 turns:** $\approx 23{,}000$ effective tokens

$$\boxed{\text{Savings} \approx 54\%}$$
