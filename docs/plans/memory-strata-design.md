...
### Prior Art (2026-05-14 update — Hindsight)

[Hindsight](https://github.com/vectorize-io/hindsight) (Vectorize.io, MIT-licensed, surfaced 2026-05-14) is the latest credible OSS agent-memory system. It self-reports state-of-the-art LongMemEval numbers (independently reproduced by Virginia Tech and The Washington Post) and is the most architecturally complete competitor surveyed.

The shape: a Docker-deployed server with a Postgres (or Oracle) backend; Python and TS clients; a wrapped-LLM integration mode plus a three-call API surface (`retain` / `recall` / `reflect`). Memories live in named **memory banks**, each configured with a mission and disposition. Inside a bank, memories sort into a biomimetic three-layer hierarchy:

- **World facts** — objective info ("the stove gets hot")
- **Experience facts** — the agent's own actions/observations ("I touched the stove and it hurt")
- **Mental models** — synthesized understanding distilled by `reflect` over the lower layers

Retrieval is **TEMPR**: four parallel passes (semantic / BM25 / graph / temporal) merged with RRF and a cross-encoder reranker. Consolidation deduplicates into durable observations with **evidence proof counts**, source quotes, and a derived freshness **trend** (stable / strengthening / weakening / stale).

What we're adopting (see specific sections below):

1. **`reflect` as a first-class operation.** Strata's Observer + Consolidator handle passive accumulation; the Retriever answers point queries. Neither does *on-demand synthesis* across existing memories ("what risks haven't we mitigated?", "what patterns explain why X keeps breaking?"). New section: "Mental Models and the `reflect` Operation."
2. **Mental models as an explicit doc type.** New `docs/mental-models/` folder. Synthesized understanding is provenance-distinct from raw entity facts and is ranked above raw facts during retrieval (matches Hindsight's "Mental Models → Observations → Raw Facts" priority).
3. **Evidence count + freshness trend in frontmatter.** Two new fields (`evidence_count`, `trend`) regenerated by the Consolidator. Strata already has `confidence` and `last_accessed`; what's missing is *accumulated positive evidence over time* and a *trajectory signal* the agent can reason about ("you've told me this 7 times" vs. "mentioned once two months ago").

What we're declining:

- **Full TEMPR (semantic + BM25 + graph + temporal + RRF + cross-encoder rerank).** Phase 3B already showed BM25-only beat BM25 + dense + RRF by 9 points on LongMemEval-S, and the cross-encoder reranker lost by 2.4 points on the same corpus. Importing the whole stack on Hindsight's say-so contradicts our own evidence. The temporal and graph pathways could plausibly earn their keep separately, but only behind their own spike — the temporal path overlaps heavily with `valid_from`/`valid_to` filters we already plan, and the graph path overlaps with wiki-link traversal that the orchestrator can already emit.
- **Server/daemon deployment shape.** Hindsight ships as a Docker image with Postgres. Wrong shape for an ax-next plugin; Strata's filesystem-first stays.
- **LLM Wrapper integration.** Hindsight's "swap your LLM client and we capture everything" pattern is convenient but bypasses Strata's deliberate Observer + confidence-gate. The dual-sourcing rule already covers the same ground more sharply (user vs. assistant origin as a confidence multiplier rather than separate pathways).
- **Memory Banks as a configurable runtime concept.** Useful primitive, but multi-bank scoping is the same problem as multi-tenant scoping (already deferred to "Open Considerations: Multi-User and Multi-Tenant Isolation"). The Strata equivalent is "the `memory/` directory IS the bank" — when multi-tenant lands, it lands as `memory/<bank>/` without renaming or restructuring anything else. Don't hardcode single-bank assumptions in path handling, but don't ship a bank manager either.

What Strata still gets right that Hindsight misses:

- Markdown-as-truth (Hindsight is Postgres/Oracle-backed; no git history, no Obsidian, no portability)
- KV-cache zone discipline (Hindsight uses provider prefix caching but no four-zone segmentation)
- Filesystem-first / plugin-shaped (Hindsight is server-shaped; deploying a Postgres alongside an ax-next plugin is the wrong cost curve)
- Single-source-of-truth pipeline (Hindsight's biomimetic split creates three parallel stores; Strata's inbox-to-docs pipeline avoids it)

The Hindsight datapoint adds one new operation (`reflect`), one new doc type (`mental-models`), and two frontmatter fields (`evidence_count`, `trend`). It does *not* alter retrieval architecture, prompt layering, or the Phase 3C orchestrator decision.
...
│   └── mental-models/        # Synthesized understanding (from `reflect`; see process #5)
│       ├── john-doe-collaboration-style.md
│       └── project-alpha-risk-pattern.md
│
...
pinned: false
evidence_count: 7               # how many corroborating observations have landed (from Hindsight)
trend: strengthening            # stable | strengthening | weakening | stale (derived; from Hindsight)
...
- **`evidence_count`** (from Hindsight) accumulates positive corroboration: each time the Observer sees a fact that the Consolidator dedupes against an existing doc-level fact, the count increments. Distinct from `confidence` (which is the Observer's per-observation extraction certainty, gated at 0.7) — `evidence_count` is *how many times this has been independently said*. A fact with `confidence: 0.8, evidence_count: 1` is plausible; the same fact at `evidence_count: 7` is durable. Surfaced in retrieval ranking as a small boost and exposed in `system/map.md` summaries as a heuristic for the orchestrator ("strong" vs. "single-source").
- **`trend`** (from Hindsight) is a derived trajectory signal regenerated by the Consolidator on each pass. Computed from the rate of new corroborations (or contradictions) over a rolling window:
  - **`stable`** — corroborations arrive at a steady rate; no contradictions
  - **`strengthening`** — accelerating corroboration (rate up over the last window)
  - **`weakening`** — corroborations slowing; possibly contradictions arriving
  - **`stale`** — no corroborations or accesses in the staleness window (default 90 days)
  Used by the orchestrator to deprioritize stale facts (don't bother loading), by the agent to caveat answers ("this is from older notes, may be out of date"), and by the Consolidator to flag stale-and-weakening facts for `reflect` review.
...
- **Orchestrator + LLM-rewritten map (c137-style) beat BM25-only by 11.2pp accuracy / 35.4pp recall@5** at n=500 (re-measured 2026-09-11 with `z-ai/glm-5.3-flash:nitro`; the superseded 7.6pp / 14.2pp figures were measured against `x-ai/grok-4.1-fast`, an id that had been 404ing for months) — the structured-retrieval stage runs *above* the indexer and does the picking, so the indexer no longer needs hybrid scoring to fuse signals. Note the whole effect lives in the planner model: the SHIPPED orchestrator model (Haiku) does not beat BM25 at all (+1.8pp, not significant). See `docs/plans/2026-09-11-orchestrator-accuracy-report.md`.
...
### 5. Reflector (from Hindsight)

The four processes above handle *accumulation*: they observe new content, retrieve relevant slices, dedupe redundant facts, and manage tiering. None of them *synthesize new understanding* across what already exists. That gap shows up the first time you ask the agent "what risks haven't we mitigated on this project?" — the answer requires reasoning across many docs (entity facts about the team, episode notes from past incidents, procedural docs that were skipped) to produce a claim that isn't written down anywhere yet.

That's what `reflect` does, and it's the operation Strata most clearly lacked before Hindsight surfaced it. Distinct from retrieval (which answers point queries against existing facts) and from consolidation (which is background dedup), `reflect` is **on-demand synthesis whose output is itself a new memory document**.

```python
class Reflector:
    """
    On-demand synthesis across existing memory. Produces a new
    mental-model document and writes it to docs/mental-models/.

    Triggered by:
    - Agent tool call: memory_reflect(question)
    - Consolidator: when stale-and-weakening facts cluster on a topic
    - Manual: via CLI, for periodic reviews
    """

    def reflect(self, question: str, scope: str = None) -> Document:
        # 1. Retrieve broadly — wider than a normal answer-shaped query
        #    because we're trying to catch indirect connections.
        candidates = self.retriever.search(
            question, top_k=20, mode="summary",
            filters={"scope": scope} if scope else None,
        )

        # 2. Load full sections for the highest-evidence_count candidates;
        #    summaries alone usually aren't enough for synthesis.
        evidence = []
        for c in candidates[:8]:
            doc = self.load_doc(c.id)
            evidence.append({
                "id": c.id,
                "summary": doc.summary,
                "facts": doc.body,
                "evidence_count": doc.frontmatter.get("evidence_count", 1),
                "trend": doc.frontmatter.get("trend", "stable"),
            })

        # 3. LLM call: synthesize a mental model.
        prompt = f"""Synthesize a mental model that answers this question.

        Question: {question}

        Evidence (drawn from the agent's existing memory):
        {format_evidence(evidence)}

        Output:
        - A short title (kebab-case slug)
        - A one-paragraph summary suitable for the frontmatter `summary` field
        - 3–8 bullet-point claims, each citing the source doc id(s) it draws from
        - 1–3 open questions or weak points where evidence is thin

        Be conservative: cite only what the evidence supports. Mark inferred
        claims as such. Do not invent doc ids.
        """
        result = self.synthesis_llm.complete(prompt)

        # 4. Write a new doc to docs/mental-models/.
        doc = Document(
            id=result.slug,
            type="mental-model",
            ...
            frontmatter={
                "type": "mental-model",
                "links": result.cited_doc_ids,
                "confidence": result.self_confidence,  # synthesis is lower-trust
                "evidence_count": 1,                   # newly minted
                "trend": "stable",
                "origin": "reflect",                   # provenance for retrieval
                "reflect_question": question,
                "reflect_scope": scope,
            },
        )
        doc.save_to(f"docs/mental-models/{doc.id}.md")
        self.reindex(doc)
        return doc

    def auto_review_stale(self):
        """
        Consolidator-driven pass: when N stale-and-weakening facts cluster
        on the same subject, run reflect() to either reaffirm them as a
        coherent (older but still valid) mental model, or surface the
        weakening trend for human review.
        """
        clusters = self.find_stale_clusters(min_cluster_size=3)
        for subject, doc_ids in clusters.items():
            self.reflect(
                question=f"What is the current state of our understanding "
                         f"of {subject}, given that several related facts "
                         f"are aging without corroboration?",
                scope=subject,
            )
```

#### Mental Models as a Doc Type

Mental models live in `docs/mental-models/` (see File System Layout). They share the standard Document Format but with these conventions:

- **`type: mental-model`** — distinguishes them from raw entity facts in retrieval and ranking
- **`origin: reflect`** — provenance signal; the agent and the orchestrator both know this is *synthesized*, not directly observed
- **`links:` is non-optional** — every mental model cites the source doc ids it was distilled from. Cited docs gain a back-reference in the link graph.
- **Lower `confidence` ceiling** — synthesis is a step removed from observation. The Reflector's self-reported confidence is capped at 0.85 by default; corroboration via downstream re-`reflect` passes can raise it.
- **No automatic supersession of source facts** — a mental model summarizing five John Doe facts does NOT replace those facts; it sits alongside them as a higher-level view. The retrieval orchestrator is taught to load mental models *first* (cheaper, denser) and drill into source facts only when the model's claims need verification.

#### Retrieval Priority: Mental Models → Observations → Raw Facts

Borrowed directly from Hindsight's three-layer hierarchy. When the Retrieval Orchestrator emits its plan against `system/map.md`, it should prefer mental-model docs over raw entity docs when the question is open-ended ("what should I know about X?", "what's the pattern here?"). The map must therefore distinguish them:

```markdown
# Memory Map

## mental-models
- [[john-doe-collaboration-style]] (★ synthesized, evidence: 12 cited facts) — Async-first, optimistic-on-deadlines, detail-oriented in reviews.
- [[project-alpha-risk-pattern]] (★ synthesized, evidence: 8 cited episodes) — Hydration bugs cluster around SSR config changes; API team is recurring blocker.

## entities/people
- [[john-doe]] — Frontend engineer at Acme Corp...
```

The `★ synthesized` marker and `evidence: N cited` count tell the orchestrator (and the agent, when the map is in context) what kind of doc it's looking at without loading it. For point queries ("what's John's email?"), the orchestrator goes straight to `entities/people/john-doe.md`. For open-ended queries ("how should I work with John on this PR?"), it loads the mental model.

#### Cost and Trigger Discipline

`reflect` is the most expensive operation in Strata: a wide retrieval (~20 candidates), full-section loads for the top 8, and a synthesis LLM call against a long prompt. Order of magnitude: 5–15× the cost of a normal `recall`. Trigger discipline matters:

- **Agent-invoked `memory_reflect(question)`** — explicit and rare; the agent must judge that synthesis is worth the cost (the tool description should say so)
- **Consolidator-invoked `auto_review_stale`** — runs at most once per consolidation pass, and only when stale clusters actually exist
- **CLI / cron invocation** — for periodic reviews ("end of week, reflect on each active project"); operator-controlled

`reflect` is *not* invoked auto-per-turn. The Observer + Retriever + Consolidator handle the steady state; `reflect` is the on-demand stage.
...
AGENT_TOOLS = {
    ...
    "memory_reflect": {
        "description": (
            "Synthesize a new mental model across existing memories. "
            "Use sparingly — this is expensive (~5-15x a normal search). "
            "Right for open-ended questions like 'what risks haven't we "
            "mitigated?' or 'what pattern explains why X keeps happening?'. "
            "Wrong for point lookups (use memory_search). Output is a new "
            "mental-model document in docs/mental-models/."
        ),
        "params": {"question": "str", "scope": "optional str"},
        "handler": reflector.reflect
    },
    "update_core_memory": {
...
> **Superseded 2026-09-11.** Every number in this Phase 3C section was measured against `x-ai/grok-4.1-fast`, whose id has since been deprecated and 404s (#515). The finding below — that the architecture works and that map quality is load-bearing — holds and was reconfirmed. The specific deltas did not transfer: re-measured on live models, the orchestrator beats BM25 by **11.2pp / 35.4pp** with `z-ai/glm-5.3-flash:nitro`, and by a **statistically insignificant +1.8pp** with the Haiku model actually shipped. See `docs/plans/2026-09-11-orchestrator-accuracy-report.md`.
...
LEVEL 3: Add Retrieval Orchestrator (one-hop XML planner)
         ...
         RE-MEASURED 2026-09-11 (that Grok id now 404s): GLM 5.3 Flash
         clears the bar by more (+11.2pp / +35.4pp); the SHIPPED Haiku
         planner does not clear it at all (+1.8pp, not significant).
...
| **Hindsight** | `reflect` operation for on-demand synthesis; mental-model doc type ranked above raw facts; `evidence_count` and `trend` frontmatter fields for accumulated corroboration |
