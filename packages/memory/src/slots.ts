 ...
 * renders the profile block must import this. The guard is
 * `scripts/__tests__/slot-vocabulary-single-owner.test.js`, and it is a
 * heuristic rather than a proof: it fails when 3+ slot names — one of them
 * `lives_in` — are mentioned within 200 characters of each other in any tracked
 * non-test JS/TS source under `packages/` or `presets/`. That anchor makes it
 * WEAKER against a partial copy: `'name' | 'role' | 'language' | 'timezone'` is
 * a second copy of part of this list and passes. That file's header lists what
 * else gets past it.
 *
