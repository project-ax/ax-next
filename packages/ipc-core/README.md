...
Transport-agnostic IPC primitives shared by the runner-IPC listeners. This package owns the dispatcher, auth, body parsing, response writers, error mapping, and per-action handlers (`handlers/llm-call.ts`, etc.). Two listener packages sit on top of it: `@ax/ipc-server` (unix-socket transport, runner-on-host) and `@ax/ipc-http` (TCP transport, runner-in-pod).
