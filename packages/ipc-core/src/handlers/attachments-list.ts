...
//
// At session start the runner enumerates the bound conversation's uploads so it
// can pull each blob (`blob.get`) and materialize the read-only working copy
// into `/ephemeral/uploads`. Each entry carries the workspace-relative `path`
// (the runner re-roots it under `<ephemeralRoot>/uploads`), the `sha256` that
// addresses the blob, and display metadata.
//
