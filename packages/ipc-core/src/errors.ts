...
export function mapPluginError(err: PluginError): HandlerErr {
  switch (err.code) {
    ...
    // Conversations bus hooks (e.g. `conversations:get`,
    // `conversations:store-runner-session`) surface these codes from
    // their ACL gates (`not-found` for cross-tenant or unknown rows,
    // `forbidden` for `agents:resolve` denial). Channel-web maps the
    // SAME bus errors as 404 / 403 over HTTP; we mirror that here so
    // the runner sees the same outcome regardless of caller. We use
