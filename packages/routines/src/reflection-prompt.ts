...
The recurrence evidence lives entirely in consolidated memory: each \`memory/docs/<category>/<slug>.md\` page carries a \`source_conversations\` field in its YAML frontmatter — the list of DISTINCT conversation ids whose observations were merged into that page. That field IS the recurrence signal, and it is the ONLY signal you use. A procedure is grounded in 2 DISTINCT past conversations only when the doc backing it has \`source_conversations\` with **2 or more entries**. If the relevant doc's \`source_conversations\` has fewer than 2 entries (or the field is absent), the procedure recurred in at most one conversation — it is NOT ready to be a skill, so leave it.
