...
export function createMemoryStrataPlugin(cfg: MemoryStrataConfig = {}): Plugin {
  ...
  //
  // This map is populated ONLY when the `onConsolidationSettleReady` seam is
  // wired (i.e. by a test). In production the seam is absent, so `lastWork`
  // stays empty — we must NOT retain a settled promise per distinct agent id
  // forever in a long-lived process for a feature production never uses.
  const settleEnabled = cfg.testHooks?.onConsolidationSettleReady !== undefined;
  const lastWork = new Map<string, Promise<unknown>>();
  // Settle: await the agent's latest consolidation work, looping until no newer
  // pass replaced it (so a settle that races a freshly-fired pass still waits
  // for it). Resolves immediately if no pass ever ran.
  const settleConsolidation = async (agentId: string): Promise<void> => {
    let awaited: Promise<unknown> | undefined;
    // Bounded loop: re-await only while the tracked promise actually changed.
    for (let i = 0; i < 100; i++) {
    ...
    async init({ bus }) {
      ...
      bus.subscribe<ChatEndPayload>('chat:end', PLUGIN_NAME, async (ctx) => {
        debouncer.schedule(ctx.agentId, async () => {
          ...
          // Gated on the seam being wired so production never retains these.
          if (settleEnabled) lastWork.set(ctx.agentId, work);
