 ...
 * deployment (one postgres table). Keying rows only by docId pooled every
 * agent's facts together — agent A's `memory_search` could return agent B's.
 * We scope every row by this key, derived from the calling agent's context, so
 * the table is partitioned per (userId, agentId).
 *
 ...
 * drift is caught by the shared `runIndexContract` isolation case. This copy is
 * byte-for-byte identical to @ax/memory-strata-index-sqlite's.
 *
