...
export const SKILL_PROPOSE_DESCRIPTOR: ToolDescriptor = {
  ...
  description: [
    ...
    '  capabilities:            # OMIT this whole block for a skill that needs no reach',
    '    allowedHosts: [api.example.com]   # bare hostnames, no scheme/path/wildcard',
    '    credentials:',
    '      - slot: EXAMPLE_API_KEY         # SCREAMING_SNAKE_CASE',
    '        kind: api-key',
    '    packages: { npm: [some-pkg] }     # optional npm/pypi name lists',
    'Capability keys (allowedHosts, credentials, mcpServers, packages) MUST be',
    'nested UNDER capabilities: — never at the top level, or the bundle is rejected',
    'as malformed.',
    '',
    'What happens next:',
    ...
    '  - A skill that needs network access or a credential is held pending the',
    "    user's approval. When they next ask you to use it, an inline card will",
    '    ask them to approve the access and enter any required key; once approved',
    '    it is ready next turn. If it needs a key, SAY SO when you propose it —',
    "    tell the user it's waiting on their approval and that they can either",
    "    approve it on first use, or approve it ahead of time from the",
    '    "My Skills" panel (where pending skills show an Approve button).',
    '',
