...
describe('buildHostTools', () => {
  ...
  it('returns an empty object when no host tools are present', () => {
    const { client } = mkClient(async () => ({ output: 'x' }));
    const tools = buildHostTools({ policy: fakePolicy(), client, tools: [SANDBOX_TOOL], holdLatch, onHold: () => {}, onToolFailure: () => {} });
  ...
  it('renders string output verbatim', async () => {
    ...
    const tools = buildHostTools({ policy: fakePolicy(), client, tools: [HOST_TOOL_A], holdLatch, onHold: () => {}, onToolFailure: () => {} });
  ...
  it('tolerates a missing description by falling back to empty string', () => {
    ...
    const { client } = mkClient(async () => ({ output: 'x' }));
    const tools = buildHostTools({ policy: fakePolicy(), client, tools: [toolNoDesc], holdLatch, onHold: () => {}, onToolFailure: () => {} });
  ...
  it('runs the executor through the policy (deny short-circuits the forward)', async () => {
    ...
    const tools = buildHostTools({ policy, client, tools: [HOST_TOOL_A], holdLatch, onHold: () => {}, onToolFailure: () => {} });
    const out = await unwrap(tools['memory.recall']?.execute)({}, OPTS);
  ...
  it('always tells the policy isBuiltin:false, even for a tool named Bash', async () => {
    ...
    const { client } = mkClient(async () => ({ output: 'ok' }));
    const tools = buildHostTools({ policy, client, tools: [hostNamedBash], holdLatch, onHold: () => {}, onToolFailure: () => {} });
