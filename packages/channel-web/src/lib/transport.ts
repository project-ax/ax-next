...
export class AxChatTransport extends HttpChatTransport<UIMessage> {
  // Note: declared as `T | undefined` (rather than `?:`) for the v2 monorepo's
  // `exactOptionalPropertyTypes: true` — assignment from optional `opts` would
  // otherwise be rejected. Stream-parsing logic is untouched.
  private onStatus: ((event: StatusEvent) => void) | undefined;
  private onRunStart: (() => void) | undefined;
  private onDiagnostic: ((d: Diagnostic) => void) | undefined;

  constructor(opts: AxChatTransportOptions = {}) {
    const user = opts.user ?? DEFAULT_USER;
    super({
      api: opts.api ?? '/api/chat/completions',
              ...
              if (pendingEventName === 'content_block') {
                try {
                  ...
                  if (block.type === 'image') {
                    ...
                    const imgTag = `\n\n![Generated image](/api/files/${block.fileId})\n\n`;
                    ...
                    const fileLink = `\n\n[${block.filename}](/api/files/${block.fileId})\n\n`;
                    controller.enqueue({ type: 'text-start', id: textPartId });
