/**
 * Decision state machine — the one piece of this prototype that is real logic
 * rather than layout, and the one piece worth locking with tests.
 *
 * Pure and time-injected: every entry point takes `now` explicitly so the same
 * functions can drive the mock server, the client's optimistic transition, and
