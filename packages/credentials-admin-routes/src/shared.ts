...
//   - 64 KiB body cap (ADMIN_BODY_MAX_BYTES) — credentials payloads are
//     small (api-keys, OAuth tokens); 64 KiB is the same generous ceiling
//     the agents admin uses.
//   - The write-error → HTTP-status mapping (PluginError code → status).
//   - The auth helper. For /admin/* we require isAdmin; for /settings/* a
...
/**
 * Translate a credentials-facade PluginError into an HTTP status. Returns
 * true if the response was written; false if the error wasn't recognized
 * (caller should re-throw so the http-server's 500 handler logs it).
 */
export function writeServiceError(res: RouteResponse, err: unknown): boolean {
  if (err instanceof PluginError) {
    ...
    if (err.code === 'not-found' || err.code === 'credential-not-found') {
