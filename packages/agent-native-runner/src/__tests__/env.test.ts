...
describe('readRunnerEnv', () => {
  ...
  it('throws MissingEnvError naming AX_IPC_SOCKET when unset', () => {
    const call = (): unknown =>
      readRunnerEnv({
        AX_SESSION_ID: 'sess-1',
        AX_AUTH_TOKEN: 'tok-123',
    ...
    try {
      ...
      expect((err as MissingEnvError).varName).toBe('AX_IPC_SOCKET');
      expect((err as MissingEnvError).message).toContain('AX_IPC_SOCKET');
    }
  });
