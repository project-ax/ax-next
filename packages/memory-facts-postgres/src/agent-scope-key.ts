...
/**
 * Per-agent scope key for the memory-facts engine (TASK-421, ported to
 * postgres by TASK-423).
 *
 * The `memory:facts:*` store is a SINGLE shared store across every agent in a
 * deployment — one sqlite db on the CLI, and on k8s one postgres TABLE inside
 * the deployment's shared database. Keying rows only by id would pool every
 * agent's facts together: agent A's `memory:facts:recall` could return agent
 * B's. We scope every row by this key, derived from the calling agent's
 ...
 * ACL that runs before every per-agent read. It IS, however, the only thing
 * keeping two tenants' rows apart inside one physical table, and on postgres
 * that table is the SHARED PRODUCTION DATABASE rather than a per-deployment
