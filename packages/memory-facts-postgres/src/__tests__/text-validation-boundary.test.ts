...
vi.mock('../schema.js', async (importOriginal) => ({
  ...
  runFactsMigration: vi.fn(() => Promise.resolve()),
}));
...
describe('@ax/memory-facts-postgres text validation before the store', () => {
  let bus: HookBus;
  let plugin: Plugin;
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    bus = new HookBus();
    bus.registerService('database:get-instance', 'test-database', async () => ({
      db: { transaction: storeTouched },
    }));
