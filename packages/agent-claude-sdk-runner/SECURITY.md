...
- **Supply chain:** One new runtime dependency: `@anthropic-ai/claude-agent-sdk`, specifier `^0.2.119`, resolved to `0.2.119` in the lockfile. `npm view @anthropic-ai/claude-agent-sdk@0.2.119 scripts` returned empty — no `preinstall` / `postinstall` / `prepare` / `install` hooks run at `pnpm install` time. Direct deps: `@anthropic-ai/sdk@^0.81.0`, `@modelcontextprotocol/sdk@^1.29.0`. Maintainers are all `*-anthropic` npm accounts (plus long-standing contributors with anthropic.com emails). Platform-specific optional binaries (`claude-agent-sdk-{darwin,linux,win32}-{arm64,x64}[-musl]`) carry the `claude` CLI binary per platform — same integrity-hashed `.tgz` shape as the parent. Peer-dep: the SDK peer-requires `zod@^4.0.0`; we ship `zod@^3.23.8`, which produces a peer-dep warning. Runtime works today (Zod v3's compat story + the SDK's passthrough-shape pattern), but we accept the warning consciously rather than silently.

## `@anthropic-ai/claude-agent-sdk` pin posture

- Current specifier: `^0.2.119`. The current sibling plugin `@ax/llm-anthropic` pins `@anthropic-ai/sdk` to an **exact** version — we should match that discipline for `claude-agent-sdk` before GA. For the Week 6.5d slice we accept the caret range because the SDK is pre-1.0 and shipping frequently; we'd rather get the latest fixes than pin to a version that'll need bumping in two weeks. Revisit when the SDK stabilizes or when we ship the first production-shaped CLI.
...
- **Wire surface:** the new env-var contract (`AX_LLM_PROXY_URL` alongside the four vars the native runner already needs) is documented in `env.ts` and carried into the child via the sandbox-provider's fixed env allowlist. No new IPC actions.
