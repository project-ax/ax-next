...
- **Sandbox:** The runner is on the sandbox side of the trust boundary. Its only reach OUT of the sandbox is (a) the IPC endpoint it was handed via `AX_RUNNER_ENDPOINT` + `AX_AUTH_TOKEN`, which carries auth-gated service calls, and (b) the credential-proxy endpoint — either `AX_PROXY_ENDPOINT` (subprocess sandbox: TCP loopback, set as `HTTPS_PROXY`) or `AX_PROXY_UNIX_SOCKET` (k8s sandbox: the runner starts a local TCP-to-unix bridge so off-the-shelf libraries can dial it). In both modes the SDK calls `api.anthropic.com` directly; the credential-proxy intercepts the request and substitutes the `ax-cred:<hex>` placeholder for the real Anthropic key (held only on the host) mid-flight. The runner never sees the real key. The runner does NOT spawn processes directly — that's the SDK's job — and the CLI's descendant tree (the `claude` CLI, its Bash / Read / Edit tool subprocesses, anything those spawn) stays inside the sandbox filesystem and process namespace. Isolation holds because the sandbox-provider plugin (subprocess today, k8s / docker later) owns the fence, not this runner. Tool reach is constrained two ways: disabled built-ins (`WebFetch`, `WebSearch`, `Task`) are blocked via `disallowedTools` AND again in the tool-name classifier as defense in depth; enabled built-ins (`Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, `TodoWrite`, and — as of Phase 0 of the skill-install workflow, I-P0-1 — `Skill`) run with `cwd` pinned to `workspaceRoot`. `settingSources: ['user', 'project']` is passed explicitly so the SDK can discover skills under `$CLAUDE_CONFIG_DIR/skills/` (host-controlled installed skills) and `<workspace>/.claude/skills/` (a narrow symlink to `<workspace>/.ax/draft-skills/`); the broader SDK-config surface the sources expose is gated by `@ax/validator-skill` at the `workspace:pre-apply` boundary (see "Newly enabled with caveats — `Skill`" below). EVERY tool call (built-in AND MCP-host) fires `tool.pre-call` via the SDK's `PreToolUse` hook, giving the host veto / rewrite authority before any execution. Every call also fires `event.tool-post-call` for observation.
...
## Newly enabled with caveats — `Skill`

**`Skill`** is the SDK's built-in skill-discovery tool. Phase 0 enables it via `allowedTools: ['Skill', ...]` plus `settingSources: ['user', 'project']`. The agent-self-escalation risk that empty `settingSources` previously prevented is now gated by three independent defenses, in order of bypass cost:

- `@ax/validator-skill` (commit `521f206c`) vetoes agent writes to the SDK config paths the new sources expose: `.claude/settings.json`, `.claude/settings.local.json`, `.claude/CLAUDE.md`, `CLAUDE.md`, `CLAUDE.local.md`, plus the dir prefixes `.claude/agents/`, `.claude/commands/`, `.claude/rules/`.
- The workspace symlink points narrowly at `<workspace>/.claude/skills → <workspace>/.ax/draft-skills`; the parent `<workspace>/.claude/` is the workspace's own directory, agent-writable only at paths the validator allows.
- `$HOME` is a per-session sandbox tempdir (subprocess) or emptyDir (k8s), isolated from the host user's `~/.claude`.

See `docs/notes/2026-05-17-sdk-setting-sources-audit.md` for the full audit of files the SDK reads from `'user'` + `'project'` sources.
...
- **`allowedTools` / `disallowedTools` and the `settingSources` veto-set are contracts, not firewalls.** A future Anthropic-introduced built-in could land outside our policy until we explicitly classify it in `tool-names.ts`; our classifier would surface it as `{kind: 'builtin'}` and pass it through to `tool.pre-call`, but that's a surprise for deny-by-default callers. Likewise, the `'user'` + `'project'` setting sources are what the SDK currently reads — if a future SDK version pulls in additional sources (machine-wide, organization, …) or starts reading a NEW file under the existing sources that bypasses validator-skill's veto path list, that's a regression we'd only catch on the SDK-bump re-audit. The runner unit test asserts `options.settingSources` is `['user', 'project']` and `options.allowedTools` contains `'Skill'` (with `'Skill'` explicitly NOT in `disallowedTools`) on every `query()` call, so a regression in our wiring fails the suite — but a regression inside the SDK itself could still be silent.
...
- Whether there's a SDK file-read path under the `'user'` or `'project'` setting source that the Task 1 audit missed. The LOW-confidence hedge in `docs/notes/2026-05-17-sdk-setting-sources-audit.md` covers this: we walked the SDK's known config-file enumerator and validator-skill vetoes everything we found, but a path that's loaded by a code path we didn't trace would slip past until the next SDK-bump re-audit catches it.
