...
// binaries the agent installs into $HOME/bin (HOME=/permanent, the git-bundled
...
//
// APPEND (not prepend) is deliberate (I5 / codex review): $HOME=/permanent is
// model-writable + restored across sessions, so prepending would let an
// injected `/permanent/bin/git` (etc.) PERSISTENTLY shadow trusted binaries.
...
describe('buildHomeBinEnv', () => {
  ...
  it('appends `:<homeDir>/bin` to a non-empty PATH', () => {
    expect(
      buildHomeBinEnv('/permanent', '/usr/local/bin:/usr/bin:/bin'),
    ).toEqual({ PATH: '/usr/local/bin:/usr/bin:/bin:/permanent/bin' });
  });
  ...
  it("puts $HOME/bin at the BACK so trusted base/venv bins win on name collisions", () => {
    ...
    const out = buildHomeBinEnv('/permanent', '/ephemeral/py/bin:/usr/bin');
    expect(out.PATH).toBe('/ephemeral/py/bin:/usr/bin:/permanent/bin');
    expect(out.PATH?.endsWith(':/permanent/bin')).toBe(true);
  });

  it('yields a bare binDir when currentPath is undefined or empty', () => {
    expect(buildHomeBinEnv('/permanent', undefined)).toEqual({ PATH: '/permanent/bin' });
    expect(buildHomeBinEnv('/permanent', '')).toEqual({ PATH: '/permanent/bin' });
  });
  ...
  it('is idempotent: returns {} when PATH already ends with binDir', () => {
    ...
    expect(
      buildHomeBinEnv('/permanent', '/usr/bin:/permanent/bin'),
    ).toEqual({});
    // Exact-match (PATH is only the binDir) is also a no-op.
    expect(buildHomeBinEnv('/permanent', '/permanent/bin')).toEqual({});
  });

  it('does not treat a different dir that merely shares a suffix as already-present', () => {
    // `/opt/permanent/bin` ends with the same basename but is NOT `/permanent/bin`
    // as a full PATH segment (the `:` boundary check guards this).
    expect(
      buildHomeBinEnv('/permanent', '/usr/bin:/opt/permanent/bin'),
    ).toEqual({ PATH: '/usr/bin:/opt/permanent/bin:/permanent/bin' });
  });
});
