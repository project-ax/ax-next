...
describe('resolveAttachmentPaths — TASK-68 ephemeral materialized uploads', () => {
  // With an uploadsRoot (the ephemeral tier), `.ax/uploads/<rest>` re-roots to
  // `<uploadsRoot>/uploads/<rest>` (the `.ax/` prefix dropped) — where uploads
  // actually materialize. This is the real production path post-TASK-68.
  it('re-roots a bare .ax/uploads path to <ephemeralRoot>/uploads (drops .ax/)', () => {
    expect(
      resolveAttachmentPaths(
        { file_path: '.ax/uploads/c/t/h__f.pdf' },
        '/permanent',
        '/ephemeral',
      ),
    ).toEqual({
      changed: true,
      input: { file_path: '/ephemeral/uploads/c/t/h__f.pdf' },
    });
  });

  it('re-roots a home-prefixed .ax/uploads path to <ephemeralRoot>/uploads', () => {
    expect(
      resolveAttachmentPaths(
        { file_path: '/home/user/.ax/uploads/c/t/h__f.pdf' },
        '/permanent',
  ...
  it('is idempotent on an already-materialized ephemeral path (changed=false)', () => {
    expect(
      resolveAttachmentPaths(
        { file_path: '/ephemeral/uploads/c/t/h__f.pdf' },
        '/permanent',
        '/ephemeral',
      ),
    ).toEqual({
  ...
  it('refuses a .. traversal even with an ephemeral root (security)', () => {
    expect(
      resolveAttachmentPaths(
        { file_path: '.ax/uploads/../../../etc/passwd' },
        '/permanent',
        '/ephemeral',
      ),
    ).toEqual({
      changed: false,
      input: { file_path: '.ax/uploads/../../../etc/passwd' },
    });
  });
});
