...
- **`oid`** — a 40-hex SHA. **Now** enforced; it used to be merely asserted.
  ...
  Two sibling fields were already safe, structurally rather than by validation, and are left alone. Note the precise claim: not "it throws before any spawn" — the engine spawns git freely before these checks, to fetch, `rev-parse` and seed — but **neither value can reach a `git` argv token until it has been string-equated to a git-minted OID**. If you grep for `spawn` to check this paragraph you will find plenty; grep for where the value itself lands in an argv array.
