...
We do make a defensive copy of incoming bytes in `applyChanges` (`impl.ts`) so a caller mutating their input buffer after `apply` returns can't poison our snapshot, and another defensive copy on the way out of `read` and `readBlobBytes` so a subscriber mutating the returned buffer can't poison a buffer we still hold a reference to. (That second copy was originally justified by what isomorphic-git might cache internally; since reads go through `cat-file` the iso-git rationale no longer applies. The copy stays — handing a caller a buffer we also retain is a bad idea regardless of who produced it.)
...
- Whether the per-repo mutex should also serialize reads. Today reads run concurrently with writes. Git objects are immutable and content-addressed, so a concurrent write cannot change an object a reader already resolved; the worst case is reading a slightly stale ref. (This used to credit isomorphic-git for that property — reads go through the git binary now, but the argument was never about which client did the reading.), but if a future backend has weaker isolation we may need to revisit.
