...
- `@ax/workspace-git` — in-process plugin for single-pod / local-CLI deployments. Imports the core directly and calls `registerWorkspaceGitHooks` at `init()` time.
- `@ax/workspace-git-http` — server-side process for multi-replica deployments. Wraps the same `registerWorkspaceGitHooks` and exposes the four hooks over HTTP. The host-side plugin in that path talks to it over the wire and never imports this core.

Both consumers share the same security profile because the code is the same; what differs is the transport in front of it. This note is the `security-checklist` walk for the core itself.
...
- **Wire surface (IPC):** the in-process consumer (`@ax/workspace-git`) has no wire surface. The HTTP consumer (`@ax/workspace-git-http`) defines its own schemas in `@ax/workspace-protocol`; those are reviewed in that package's SECURITY.md, not here.
