 ...
 * under /agent/workspace/** or /agent/.ax/artifacts/**; the
 ...
 * because only it has filesystem access to /agent at call time.
...
export const ARTIFACT_PUBLISH_DESCRIPTOR: ToolDescriptor = {
  ...
  description: [
    ...
    '  - /agent/workspace/**  (publish a snapshot of versioned project content)',
    '',
  ...
  inputSchema: {
    ...
    properties: {
      path: {
        type: 'string',
        description:
          'Absolute path under /ephemeral/artifacts/ (or /agent/workspace/) to publish.',
      },
      displayName: {
        type: 'string',
